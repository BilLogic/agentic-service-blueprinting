...
Resize two containers so the type floor can rise to 12px.
...
mono with tabular figures, at `xs`. The slide badge keeps its chip but sizes
to its content (`h-5 min-w-5 px-1`), so one digit stays a circle and two or
more grow it. The storyboard strip no longer draws a per-frame lane name; the
