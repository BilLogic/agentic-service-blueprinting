...
404 sits above the catch-all now; `:splat` keeps the real path, so an asset that
...
`npm run check:hosting` holds both: that the 404 precedes the catch-all, an
order a diff reads as correct either way, and that the long cache covers the
hashed output alone.
