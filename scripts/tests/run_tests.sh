...
pass "fallback-reregister (re-registration is a no-op — idempotent)"
...
# Restore the shipped registry state and confirm it still type-checks.
cp "$TMP/blueprintFallbacks.ts.bak" "$REGISTRY"
rm -f "$GENERATED_TS"
npx tsc -p tsconfig.app.json > "$TMP/tsc3.out" 2>&1 \
  || fail "restore-tsc: default registry no longer type-checks — $(tail -20 "$TMP/tsc3.out")"
pass "restore-tsc (default scale-fixture registry restored and type-checks)"
