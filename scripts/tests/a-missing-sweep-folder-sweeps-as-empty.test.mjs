...
test('a tree with docs/ and no plugin surface sweeps its docs and nothing else', () => {
  ...
  try {
    assert.deepEqual(sweptDocs(t.root, PLUGIN_DIRS), [
      ...ROOT_DOCS,
...
test('a tree with none of the swept folders sweeps its root docs alone', () => {
  ...
  try {
    assert.deepEqual(sweptDocs(t.root, PLUGIN_DIRS), ROOT_DOCS)
  } finally {
    t.done()
  }
})
...
test('a folder that exists is still swept beside one that does not', () => {
  ...
  try {
    assert.deepEqual(sweptDocs(t.root, PLUGIN_DIRS), [...ROOT_DOCS, 'skills/map/SKILL.md'])
  } finally {
    t.done()
  }
})
