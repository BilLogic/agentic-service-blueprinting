...
def blueprint_data_for_path(scenario: dict, path: dict) -> dict:
    ...
    return {
        ...
        "dependencies": [project(seed_trigger_fields(edge)) for edge in path["triggers"]],
    }
...
def emit_module(model: dict, ir_name: str) -> str:
    ...
    dimensions = "\n".join(
        ...
        f"{sum(len(p['triggers']) for p in scenario['paths'])} dependencies"
        for scenario in model["scenarios"]
    )
...
def main(argv=None) -> int:
    ...
    for scenario in model["scenarios"]:
        ...
        print(
            ...
            f"{len(scenario['paths'])} paths, {len(scenario['steps'])} steps, {cells} cells, {triggers} dependencies"
        )
