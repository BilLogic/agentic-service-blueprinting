 ...
 *
 * COOKIES ARE THE THIRD STORE, and they were added before a defect rather
 * than after one — which is worth saying, because every other line in this set
 * was written after something was untrue. The subject was the two web-storage
 * APIs, so the bare `sidebar_state` the vendored sidebar wrote sat outside both
 * the seam and this script while every stated rule read as if it were covered.
 * That cookie is deleted rather than namespaced — nothing read it — so the
 * application currently sets no cookie at all, and this half of the check
 * guards the DIRECTORY it came out of: `src/components/ui/` is generated by the
 * shadcn CLI, and a re-vendor that restores the write goes red here instead of
 * putting a name back in a jar shared by every installation on the origin.
 *
 * ── The subject ────────────────────────────────────────────────────────────
 *
 ...
 * interpolated at its head (`` `${COOKIE_NAME}=${open}` ``), which are the two
 * shapes a reader writes; a name assembled any other way is refused below
 * rather than guessed at. Compound assignment is read too (`+=`, `||=`): `+=`
 * cannot set a coherent cookie — the name it produces is the whole serialized
 * jar — but it REACHES the jar, and a write this check declined to judge on the
 * grounds that it was a bad way to write is a hole with an excuse in it.
 * Reading the jar is not a write and names no cookie, so `document.cookie` as a
 * value is not read.
 *
 ...
 * else — `const jar = document`, in either spelling — is DENIED where the write
 * is, because a receiver this script cannot resolve to `document` is a write
 * whose cookie name it cannot judge.
 */
 ...
 * the reason the stores above are. Compound assignment counts, because it
 * reaches the jar; `==` does not, because a comparison against the jar reads it
...
const ESCAPED_STORE = [
  ...
  },
  {
    ...
    test: /(?<!\bdocument\s*)(?:\.\s*cookie|\[\s*['"]cookie['"]\s*\])\s*(?:\+|\|\||&&|\?\?)?=(?!=)/,
  },
]
