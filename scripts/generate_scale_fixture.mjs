...
const KIND = { path: 0, layer: 1, step: 2, cell: 3, trigger: 4, slice: 5, sliceItem: 6 }
...
const FIXTURE_TIMESTAMP = '2026-08-17T00:00:00+00:00'
...
// ---------------------------------------------------------------------------
// Demo slices — two derived-layer rows over the happy path, so the slices
// surface (sidebar groups, focus view, presentation) renders with zero
// config. Cells are picked with the same laneHasCell predicate that placed
// them, so the members always resolve.
// ---------------------------------------------------------------------------

const HAPPY = PATHS[0]

function demoCellRef(row, col) {
  if (!laneHasCell(row, col, HAPPY)) {
    throw new Error(`demo slice references missing cell (row ${row}, col ${col})`)
  }
  const lane = LANES.find((entry) => entry.row === row)
  return {
    id: fid(HAPPY.ordinal, KIND.cell, row, col),
    key: cellKeyFor(HAPPY.name, lane.name, STEP_NAMES[col - 1]),
  }
}
...
function buildDemoSlices() {
  ...
  const item = (sliceOrdinal, sliceId, position, caption, narrative, refs) => ({
    id: fid(0, KIND.sliceItem, sliceOrdinal, position),
    slice_id: sliceId,
    position,
    caption,
    narrative,
  ...
  const journeyId = fid(0, KIND.slice, 1, 0)
  const journey = {
    slice: {
      id: journeyId,
      service_lifecycle_id: LIFECYCLE_ID,
      title: 'Requester updates, end to end',
      description:
        'Every touchpoint where the frontstage keeps the requester informed, from acknowledgement to closure.',
      actor: 'Frontstage Ops',
      slice_type: 'journey',
      origin: 'generated',
      locale: 'en',
      position: 1,
    ...
    items: [
      item(1, journeyId, 1, 'Acknowledge the request', 'The request is triaged and the requester hears back that work is underway.', [demoCellRef(5, 2), demoCellRef(5, 3)]),
      item(1, journeyId, 2, 'Report from the field', 'Inspection and repair progress is relayed while the crew is on site.', [demoCellRef(5, 6), demoCellRef(5, 7)]),
      item(1, journeyId, 3, 'Confirm and close', 'Quality review completes and the requester gets the closing notice.', [demoCellRef(5, 10), demoCellRef(5, 11)]),
    ],
  }

  const stepId = fid(0, KIND.slice, 2, 0)
  const step = {
    slice: {
      id: stepId,
      service_lifecycle_id: LIFECYCLE_ID,
      title: 'The on-site inspection moment',
      description:
        'One step read vertically: who acts and which systems carry the inspection.',
      actor: null,
      slice_type: 'step',
      origin: 'generated',
      locale: 'en',
    ...
    items: [
      item(2, stepId, 1, 'People at the step', 'The actors coordinating the inspection on the ground.', [demoCellRef(1, 6), demoCellRef(3, 6)]),
      item(2, stepId, 2, 'Systems at the step', 'The tech and frontstage work carrying the same moment.', [demoCellRef(4, 6), demoCellRef(5, 6)]),
    ],
  }
...
export const SCALE_TEST_LIFECYCLE_ID = '${LIFECYCLE_ID}'
${blueprints
  .map(
    ({ path, blueprint }) =>
...
const sqlUuidArray = (ids) =>
  `array[${ids.map((id) => `${q(id)}::uuid`).join(', ')}]`
const sqlTextArray = (values) =>
  `array[${values.map((value) => q(value)).join(', ')}]`
