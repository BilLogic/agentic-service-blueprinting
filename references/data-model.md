...
| `slides` | One slide of a slice | `position` (unique per slice, deferrable), `cell_ids`/`cell_keys` (equal cardinality enforced; empty = title-only divider slide), `title`, `caption`, `illustration` JSONB — full-replacement semantics on rework |
