...
| **Enables** | `cell_dependencies.kind` | The word IS the value — `kind = 'enables'`, the recorded dependency that never draws — and `kind` is the name of the place holding it. It needs no second label for the other end: `enables` reads source-first whichever end you stand at. |
