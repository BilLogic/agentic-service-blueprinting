  ...
  "group/alert relative grid w-full gap-1 rounded-lg border px-2 py-2 text-left text-sm has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        default:
          "bg-card text-card-foreground *:[svg]:rounded-sm *:[svg]:bg-foreground *:[svg]:p-1 *:[svg]:text-contrast",
        ...
        destructive:
          "border-border-destructive bg-surface-destructive text-foreground *:[svg]:rounded-sm *:[svg]:bg-destructive *:[svg]:p-1 *:[svg]:text-destructive-foreground",
        warning:
          "border-border-warning bg-surface-warning text-foreground *:[svg]:rounded-sm *:[svg]:bg-warning *:[svg]:p-1 *:[svg]:text-warning-foreground",
        info: "border-border-info bg-surface-info text-foreground *:[svg]:rounded-sm *:[svg]:bg-info *:[svg]:p-1 *:[svg]:text-info-foreground",
        success:
          "border-border-success bg-surface-success text-foreground *:[svg]:rounded-sm *:[svg]:bg-success *:[svg]:p-1 *:[svg]:text-success-foreground",
      },
    },
    defaultVariants: {
      variant: "default",
