 ...
 *
 * An item states its row height through `size`, because the benchmark's 44px
 * row is right for a palette and too tall for the menus that also compose this
 * primitive; a per-caller `py-*` override would have been the same number
 * spelt in two places.
 *
 * The list's cap is the benchmark's 500px, written as a literal rather than a
 * layout constant: the rule that layout numbers are declared in TypeScript and
 * handed to CSS is for numbers the runtime does arithmetic on, and nothing
 * measures against this one.
 *
    ...
    <Dialog {...props}>
      <DialogContent
        className={cn(
          ...
          "top-1/3 max-h-[500px] translate-y-0 overflow-hidden rounded-lg border border-border p-0 shadow-md ring-0 sm:max-w-xl",
          className
        )}
  ...
  return (
    ...
    >
      ...
      <CommandPrimitive.Input
        ...
        className={cn(
          // The base layer rings every focusable element that states no rule
          // of its own (`base.css`), which on a borderless search field draws
          // the input plate this surface exists without. Opted out here, the
          // way the benchmark's own input is.
          "h-12 w-full border-0 bg-transparent text-sm shadow-none outline-hidden placeholder:text-tertiary-foreground focus-visible:border-0 focus-visible:shadow-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50",
          className
        )}
        {...props}
      />
...
const COMMAND_ITEM_SIZES = {
  /** The benchmark's palette row. */
  default: "py-3",
  /** A menu row, for the dropdowns that compose this primitive too. */
  sm: "py-2",
} as const
...
function CommandItem({
  ...
  size?: keyof typeof COMMAND_ITEM_SIZES
}) {
  return (
    <CommandPrimitive.Item
      data-slot="command-item"
      className={cn(
        "group/command-item relative flex cursor-default items-center gap-2 rounded-md px-2 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-selected:bg-accent data-selected:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-selected:*:[svg]:text-foreground",
        COMMAND_ITEM_SIZES[size],
        className
      )}
      {...props}
    />
