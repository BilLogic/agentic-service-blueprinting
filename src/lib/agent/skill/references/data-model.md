...
| `slide_images` | One member of a slide's authored image set | Exactly one of `cell_id` or `image_url`; unique `(slide_id, position)`. Replaced as a set, not patched. Empty with `shows_all_images` false means show nothing |
