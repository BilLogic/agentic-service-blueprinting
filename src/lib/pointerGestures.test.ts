...
describe('a drag publishes once a frame', () => {
  it('publishes nothing until the frame turns', () => {
    ...
    const publish = vi.fn()
    const queue = createFramePatchQueue<{ x?: number }>(publish, frames.scheduler)

    queue.schedule('mark', { x: 1 })
  ...
  it('merges every sample of one subject into one write', () => {
    ...
    queue.schedule('mark', { x: 1, y: 1 })
    queue.schedule('mark', { x: 2 })
    queue.schedule('mark', { x: 3 })
  ...
  it('never merges two subjects', () => {
    ...
    const queue = createFramePatchQueue<{ x?: number }>(
      (id, patch) => void published.push([id, patch]),
      frames.scheduler,
    )

    queue.schedule('first', { x: 1 })
    queue.schedule('second', { x: 2 })
